...
type
  ...
  public
    ...
    procedure Atualizar(const objetoPessoa: TPessoa);
  end;
...
procedure TControllerPessoa.Atualizar(const objetoPessoa: TPessoa);
begin
  if objetoPessoa.NomePessoa = '' then
    raise Exception.Create('Preencha o nome da pessoa.');

  if (objetoPessoa.CPF = '') or (objetoPessoa.CPF = '   .   .   -  ') then
    raise Exception.Create('Preencha o CPF da pessoa.');

  if not (ValidarCPF(objetoPessoa.CPF)) then
    raise Exception.Create('CPF inválido!');

  objetoPessoa.Atualizar(objetoPessoa);
end;
